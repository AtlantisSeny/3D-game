	...
	void OnGUI () {
        ...
        if (game_start) {
            ...
            if (action.GetRound() == 3 && action.GetTrial() == 10) {
                ...
                if (GUI.Button(new Rect(Screen.width / 2 - 40, Screen.height / 2 - 70, 80, 30), "物理学模式"))
                {
                    mode = 1;
                    action = SSDirector.GetInstance().CurrentScenceController as IUserAction;
                }
                if (GUI.Button(new Rect(Screen.width / 2 - 40, Screen.height / 2 - 20, 80, 30), "运动学模式"))
                {
                    mode = 0;
                    action = SSDirector.GetInstance().CurrentScenceController as IUserAction;
                }
                if (GUI.Button(new Rect(Screen.width / 2 - 50, Screen.height / 2 + 30, 100, 50), "重新开始")) {
                    action.ReStart();
                    return;
                }
                action.GameOver();
            ...
            if (GUI.Button(new Rect(Screen.width / 2 - 40, Screen.height / 2 +120, 80, 30), "运动学模式"))
            {
                mode = 0;
            }
        }
